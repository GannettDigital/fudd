...
var Fudd = {
    ...
    teardown: function(config, finalCallback) {
        Fudd._connect(config, function(connection, channel) {
            palinode.mapEach(config.exchanges, Fudd._deleteExchange.bind(null, channel), function(err) {
                if (err) return finalCallback(err);

                palinode.mapEach(config.queues, Fudd._deleteQueue.bind(null, channel), function(err) {
                    if (err) return finalCallback(err);
